...
function charts() {
    ...
    if ($(".verticalChart")) {
        $(".singleBar").each(function () {
            ...
            $(this).find(".value").animate({height: i}, 2000, function () {
    ...
    if ($(".main-chart")) {
        $(".bar").each(function () {
            ...
            $(this).find(".value").animate({height: i}, 2000)
        })
    }
    if ($(".bar-stat").length) {
        if (retina()) {
    ...
    if ($(".chart-type1").length) {
        $(".chart-type1").each(function () {
            ...
            var K = $.plot($(".chart-type1"), [{
                data: i,
                label: "Visits",
                lines: {show: true, fill: true, fillColor: rgbToRgba(I, 0.25), lineWidth: 3},
                points: {show: true, lineWidth: 3, fill: true},
                shadowSize: 0
            }, {
                data: M,
                bars: {show: true, fill: false, barWidth: 0.1, align: "center", lineWidth: 8}
            }], {
    ...
    if ($(".multi-stat-box-chart").length) {
        $(".multi-stat-box-chart").each(function () {
            ...
            var M = $.plot($(".multi-stat-box-chart"), [{data: L}], {
                series: {
                    lines: {
                        show: true,
                        lineWidth: 3,
                        fill: false
                    }, points: {show: true, lineWidth: 3, fill: true, fillColor: "#fff"}, shadowSize: 0
                },
                grid: {hoverable: true, clickable: true, tickColor: "#fff", borderColor: false},
                colors: ["#c7cbd5"],
                xaxis: {
                    mode: "time", tickFormatter: function (Q, P) {
                        return i[new Date(Q).getDay()]
                    }, color: "#c7cbd5", autoscaleMargin: 1e-18
                },
                yaxis: {ticks: 4, tickDecimals: 0, color: "#fff"}
            });
    ...
    if ($(".chart-type2").length) {
        $(".chart-type2").each(function () {
            ...
            var i = $(this).parent().parent().css("color");
            var K = $.plot($(".chart-type2"), [{data: I}], {
                series: {
                    lines: {show: true, lineWidth: 3, fill: false},
                    points: {show: true, lineWidth: 3, fill: true, fillColor: i},
                    shadowSize: 0
                },
                grid: {
                    hoverable: true,
                ...
                colors: ["#fff"],
                xaxis: {ticks: 6, tickDecimals: 0, tickColor: i, color: "#fff"},
                yaxis: {ticks: 4, tickDecimals: 0, color: "#fff", autoscaleMargin: 0.000001}
            });
    ...
    if ($("#stats-chart2").length) {
        ...
        var z = $.plot($("#stats-chart2"), [{data: k, label: "Tickets"}, {
            data: j,
            ...
            series: {
                ...
                }, points: {show: false, lineWidth: 1}, shadowSize: 0
            },
    ...
    if ($("#facebookChart").length) {
        ...
        var z = $.plot($("#facebookChart"), [{data: o, label: "Fans"}], {
            series: {
                lines: {
                    show: true,
                    lineWidth: 2,
                ...
                }, points: {show: true, lineWidth: 2}, shadowSize: 0
            },
    ...
    if ($("#twitterChart").length) {
        ...
        var z = $.plot($("#twitterChart"), [{data: h, label: "Followers"}], {
            series: {
                lines: {
                    show: true,
                    lineWidth: 2,
    ...
    if ($("#activeUsers").length) {
        ...
        function C() {
            $.plot($("#activeUsers"), [H], {
                ...
                grid: {hoverable: true, clickable: true, tickColor: "#f6f6f6", borderWidth: 0},
                colors: ["#CBE968"],
                xaxis: {
                    ticks: 0, tickDecimals: 0, tickFormatter: function (I, i) {
                        return I
                    }
                },
                yaxis: {
                    ticks: 5, tickDecimals: 0, tickFormatter: function (i) {
                        return i
                    }
                }
            })
    ...
    if ($("#stats-chart").length) {
        ...
        var z = $.plot($("#stats-chart"), [{data: r, label: "visitors"}], {
            series: {
                lines: {
                    show: true,
                    lineWidth: 3,
    ...
    if ($("#sincos").length) {
        ...
        var z = $.plot($("#sincos"), [{data: a, label: "sin(x)/x"}, {
            data: d,
            label: "cos(x)"
        }], {
            series: {lines: {show: true, lineWidth: 2}, points: {show: true}, shadowSize: 2},
    ...
    if ($("#flotchart").length) {
        ...
        $.plot($("#flotchart"), [{label: "sin(x)", data: H}, {label: "cos(x)", data: F}, {
            label: "tan(x)",
            data: D
        }], {
            ...
            grid: {tickColor: "#dddddd", borderWidth: 0},
    ...
    if ($("#stackchart").length) {
        ...
        function e() {
            $.plot($("#stackchart"), [H, F, D], {
                series: {
                    stack: l,
                    lines: {show: b, fill: true, steps: B},
                    bars: {show: s, barWidth: 0.6}
                }, colors: ["#FA5833", "#2FABE9", "#FABB3D"]
            })
        }
    ...
    var G = [{label: "Desktop", data: 73}, {label: "Mobile", data: 27}];
    if ($("#deviceChart").length) {
        $.plot($("#deviceChart"), G, {
            series: {pie: {innerRadius: 0.6, show: true}},
            legend: {show: true},
            colors: ["#FA5833", "#2FABE9", "#FABB3D", "#78CD51"]
        })
    }
    var G = [{label: "iOS", data: 20}, {label: "Android", data: 7}, {label: "Linux", data: 11}, {
        label: "Mac OSX",
        data: 14
    ...
    if ($("#serverLoad").length) {
        var f = {
            ...
            yaxis: {
                min: 0, max: 100, tickFormatter: function (i) {
                    return i + "%"
                }
            },
            xaxis: {show: false},
            colors: ["#FA5833"],
            grid: {tickColor: "#f9f9f9", borderWidth: 0}
        };
...
/**
 * Created by Babajide Owosakin on 3/3/2015.
 */
